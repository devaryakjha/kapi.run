import type { Address, CartLine, KapiSession, ManualFallbackSummary, Restaurant } from '@kapi/spec'
...
export const API_URL = import.meta.env.VITE_KAPI_API_URL ?? 'http://127.0.0.1:3001'
...
export async function encryptSession(session: KapiSession, key: string) {
...
export async function decryptSession(ciphertext: string, key: string) {
...
export function localSessionKey(sessionId: string) {
...
export function makeManualFallback(session: KapiSession): ManualFallbackSummary {
...
export function SetupStep({ title, active, status, children }: { title: string; active?: boolean; status?: string; children: React.ReactNode }) {
  return (
